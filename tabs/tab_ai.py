...
class OllamaLLMAdapter:
    ...
    def __init__(self, api_base: str, model: str):
        ...
        if "/api/generate" in self.api_base:
            ...
            self.api_base = self.api_base + "/api/chat"
    ...
    def chat(self, messages: List[Dict[str, str]], temperature: float = 0.3,
        ...
        try:
            response = requests.post(self.api_base, json=data, timeout=120)
...
class EnhancedTabAI():
    ...
    # ========= 收集文件 + 运行缺陷检测 ==========
    def _collect_uploaded_files(self) -> List[Dict[str, str]]:
        """
        从UI收集上传的文件
        返回格式: [{"file": "filename.py", "content": "..."}, ...]
        """
        files = []
        ...
        # 从 input_edit 收集（输入框1）
        if hasattr(self.ui, 'input_edit'):
            print(f"[DEBUG] input_edit 存在")
            if hasattr(self.ui.input_edit, 'uploaded_files'):
                print(f"[DEBUG] input_edit.uploaded_files 存在，数量: {len(self.ui.input_edit.uploaded_files)}")
                for i, file_info in enumerate(self.ui.input_edit.uploaded_files):
                ...
                print(f"[DEBUG] input_edit.uploaded_files 不存在")
        else:
            print(f"[DEBUG] input_edit 不存在")

        # 从 input_edit_1 收集（输入框2）
        if hasattr(self.ui, 'input_edit_1'):
            print(f"[DEBUG] input_edit_1 存在")
            if hasattr(self.ui.input_edit_1, 'uploaded_files'):
                print(f"[DEBUG] input_edit_1.uploaded_files 存在，数量: {len(self.ui.input_edit_1.uploaded_files)}")
                for i, file_info in enumerate(self.ui.input_edit_1.uploaded_files):
                    print(f"[DEBUG] 文件 {i + 1}: {file_info}")
                    files.append({
                        "file": file_info.get("name", "unknown"),
                        "content": file_info.get("content", ""),
                ...
                print(f"[DEBUG] input_edit_1.uploaded_files 不存在")
        else:
            print(f"[DEBUG] input_edit_1 不存在")

        print(f"[DEBUG] 收集完成，总文件数: {len(files)}")
        if files:
            print(f"[DEBUG] 第一个文件: {files[0]}")
    ...
    def run_multi_agent_workflow(self):
        ...
        try:
            self.ui.output_area.append("\n" + "=" * 80)
            self.ui.output_area.append("🚀 启动多语言多Agent协作修复系统")
            self.ui.output_area.append("=" * 80)
            self.ui.output_area.repaint()
            QApplication.processEvents()
            ...
            try:
                ...
                for lang, file_list in classified.items():
                    if file_list and lang != Language.UNKNOWN:
                        ...
                        lang_stats.append(f"{lang_info['name']}: {len(file_list)}个")

                if lang_stats:
                    self.ui.output_area.append(f"   语言分布: {' | '.join(lang_stats)}")
            except Exception as e:
            ...
            # ===== 步骤2：获取用户需求 =====
            user_request = ""
            if hasattr(self.ui.input_edit, "get_user_input"):
                user_request = self.ui.input_edit.get_user_input()

            if not user_request.strip():
                user_request = self.ui.input_edit.toPlainText().strip()

            if user_request:
                self.ui.output_area.append(f"\n📝 用户需求: {user_request[:100]}...")

                # ===== 步骤3：配置LLM（修复部分） =====
                llm_client = None
                api_base = self.ui.conf_api_base.text().strip()
                api_key = self.ui.conf_api_key.text().strip()
                model = self.ui.conf_model.text().strip()
                ...
                if api_base and model:
                    try:
                        ...
                        if "11434" in api_base or "ollama" in api_base.lower():
                            ...
                            self.ui.output_area.append(f"✅ LLM已配置: Ollama - {model}")
                        elif api_key:
                            # 使用 OpenAI 客户端
                            from openai import OpenAI
                            llm_client = OpenAI(
                            ...
                            self.ui.output_area.append(f"✅ LLM已配置: OpenAI - {model}")
                        else:
                            self.ui.output_area.append("⚠️ 未提供API Key，将仅使用规则修复")
                    except Exception as e:
                        self.ui.output_area.append(f"⚠️ LLM配置失败: {e}")
                else:
            ...
            input_data = {
                ...
                "test_cases": []  # 可扩展：从UI收集测试用例
            }

            # 执行工作流
            self.ui.output_area.append("\n" + "=" * 80)
            self.ui.output_area.append("▶️ 开始执行多Agent协作流程")
            self.ui.output_area.append("=" * 80)
            self.ui.output_area.repaint()
            QApplication.processEvents()
            ...
            if exec_time:
                ...
                for stage, duration in exec_time.items():
                    self.ui.output_area.append(f"   - {stage}: {duration:.2f}秒")
            ...
            if fixed_files:
                reply = QMessageBox.question(
                    self.ui,
                    "保存修复后的代码",
                    f"成功修复了 {len(fixed_files)} 个文件。\n\n"
                    f"是否保存修复后的代码？\n\n"
                    f"选择 Yes：选择目录保存所有修复后的文件\n"
                    f"选择 No：仅显示结果，不保存\n\n"
                    f"⚠️ 注意：这不会覆盖原文件，而是保存到新目录",
                    QMessageBox.Yes | QMessageBox.No,
                    QMessageBox.No
                )
