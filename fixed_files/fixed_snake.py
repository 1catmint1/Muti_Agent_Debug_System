...
class Snake(cocos.cocosnode.CocosNode):
    ...
    # Intentional: function that would raise IndexError if called (out-of-range)
    def maybe_index_error(self):
        # not called by default; present to be caught by dynamic tests
        return self.body[9999]

    def maybe_shell_call(self):
        """Intentional: subprocess call with shell=True (security smell).
        Not called by default; present for dynamic detection exercises."""
        import subprocess
        # benign command; uses shell=True to be flagged by scanners
